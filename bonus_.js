(function (window, undefined) {
	...
	var GS_Templates = {
		...
		tempFrame: function(){
			var templates = '<section class="bonus_frame success">' +
							...
							'        </dl>' +
							'    </div>' +
							'    <a class="this_btn" href="/you/comment">继续点评开宝箱</a>' +
	...
	var StartInit = {
		init: function(){
			var GS_DOM = document.getElementById("dom"),
				$pageId = $("#page_id");
			switch (M_INFO.URL_TYPE()){
				case 0:
					if(M_INFO.UserID()){
						GS_DOM.innerHTML = GS_Templates.tempHeader() + GS_Templates.tempList() + GS_Templates.tempIndex() + GS_Templates.tempFooter();
					}else{
						GS_DOM.innerHTML = GS_Templates.tempHeader() + GS_Templates.tempIndex() + GS_Templates.tempFooter();
					}
					$pageId.val(PageID.Web[0]);
					break;
				case 1:
					GS_DOM.innerHTML = GS_Templates.tempHeader() + GS_Templates.tempList() + GS_Templates.tempResult() + GS_Templates.tempFooter() + GS_Templates.tempFrame();
					$pageId.val(PageID.Web[1]);
					break;
			}
		...
		getMyBonusData: function(){
			$.ajax({
				url: 'http://m.fat19.qa.nt.ctripcorp.com/you/api/CommentOpenJewelBoxapi/GetAwardWinningStatListByUserId',
				type: 'GET',
				data: {
					Auth: M_INFO.UserID()
				},
		...
		getUserListData: function(){
			$.ajax({
				url: 'http://m.fat19.qa.nt.ctripcorp.com/you/api/CommentOpenJewelBoxapi/GetLatestAwardWinninglist',
				type: 'GET',
				data: {
					topNumber: 10
				},
		...
		getLotteryData: function(images, boxSite){
			$.ajax({
				url: 'http://m.fat19.qa.nt.ctripcorp.com/you/api/CommentOpenJewelBoxapi/LuckyDraw',
				type: 'GET',
				data: {
					Auth: M_INFO.UserID(),
